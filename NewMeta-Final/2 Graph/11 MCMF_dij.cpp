...
LL aug(int v, LL flow) { // 这里是多路增广才要抄的
    if(v == T) return flow;
    vst[v] = 1; LL flow1(flow);
    for(int & i(cur[v]); i < (int)G[v].size(); i++) {
        Edge & e = G[v][i];
        if(e.f && d[v] + e.c == d[e.to] && !vst[e.to]) {
            LL flow1(aug(e.to, min(flow, e.f)));
            flow -= flow1; e.f -= flow1;
            G[e.to][e.r].f += flow1;
        }
        if(flow == 0) {
            vst[v] = 0; return flow1 - flow;
        }
    }
    return flow1 - flow;
}
LL mcmf() {
    ...
    while (SPFA(S)) {
        ...
        /*LL f = INF; // 这里是单路增广
         ...
         int u = prv[v];        int j = prp[v];
         f = min(f, G[u][j].f);
         } for (int v = T; v != S; v = prv[v]) {
         int u = prv[v];        int j = prp[v];
         G[u][j].f -= f;        G[v][G[u][j].r].f += f;
         } sT += d[T]; ans += f * sT;*/
        for(int i(1); i <= T; i++)
            for(auto & e : G[i])
                e.c += d[i] - d[e.to];
