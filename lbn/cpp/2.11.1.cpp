...
void lk(int x,int y){mrt(x);F[x]=y;}//无向图 
void lk(int x,int y){acs(x);sy(x);F[x]=y;}//有向图 
void ct(int x,int y){mrt(x);acs(y);sy(y);c[y][0]=F[x]=0;}//无向图
void ct(int x){acs(x);sy(x);F[c[x][0]]=0;c[x][0]=0;ps(x);}//有向图
void add(int x,int y,int z){mrt(x);acs(y);sy(y);ADD(y,z);}
int qu(int x,int y){mrt(x);acs(y);sy(y);return V[y];}//无向图 
int qu(int x){acs(x);sy(x);return V[x];}//有向图 
